...
fn main() {
    App::new()
        ...
        .add_system_set(SystemSet::on_enter(AppState::Winner).with_system(open_menu_system))
        .add_system_set(SystemSet::on_update(AppState::Winner).with_system(button_system))
        .add_system_set(SystemSet::on_exit(AppState::Winner).with_system(restart_system))
        .run();
}
...
fn open_menu_system(
    ...
    let winner_text = format!("Player {} wins!", next_player.mark);
    commands
...
fn restart_system(
    ...
    mut mark_query: Query<Entity, (With<Marks>)>,
) {
    // close menu
    ...
    // erase marks
    for mark in mark_query.iter_mut() {
        ...
        match *interaction {
            Interaction::Clicked => {
                *color = PRESSED_BUTTON.into();
                // Must reset click otherwise counts towards game
